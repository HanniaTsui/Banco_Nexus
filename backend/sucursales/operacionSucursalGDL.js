...
const registrarTransaccion = async (cuenta, monto, tipo) => {
  ...
  try {
    ...
    console.log(`Transacción ${tipo} de $${monto} realizada en Guadalajara para cuenta ${cuenta}. Nuevo saldo: ${cuentaActualizada.saldo}`);
  } catch (error) {
    console.error('Error en Guadalajara:', error);
  } finally {
    await client.close();
  }
};
